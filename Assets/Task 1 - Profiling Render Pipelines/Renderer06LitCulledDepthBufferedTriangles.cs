 ...
 * of rendering a model using depth buffered, culled, 
 * Phong lit triangles
 *
...
public class Renderer06LitCulledDepthBufferedTriangles : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        depthBufferTexture = new Texture2D(renderedTextureSize.x, renderedTextureSize.y);
    ...
    void LateUpdate()
    {
        ...
        // Render our projected mesh vertices in wireframe
        RenderProjectedVerticesAsCulledPhongLitTriangles(projectedVertices, originalMeshTriangles, vertexColours, renderingCamera.transform.position);
    ...
    void RenderProjectedVerticesAsCulledPhongLitTriangles(Vector3[] projectedVertices, int[] meshTriangles, Color[] vertexColours, Vector3 cameraWorldPosition)
    {
        ...
        for (int i = 0; i < meshTriangles.Length; i += 3)
        {
                ...
                (proj_v3.x >= -1 && proj_v3.x <= 1 && proj_v3.y >= -1 && proj_v3.y <= 1))
            {
                ...
                if (Vector3.Dot(proj_triangleNormal, proj_triangleCentre - cameraWorldPosition) <= 0)
                {
                    ...
                    DrawInterpolatedTriangle(frameBufferTexture, texturespace_v1, texturespace_v2, texturespace_v3,
        ...
        for (int y = triangleBoundingRect.yMin; y <= triangleBoundingRect.yMax; y++)
        {
            for (int x = triangleBoundingRect.xMin; x <= triangleBoundingRect.xMax; x++)
            {
                ...
                if (w_v1 >= 0 && w_v2 >= 0 && w_v3 >= 0)
                {
                    ...
                    float depthValue = v1.z * w_v1 + v2.z * w_v2 + v3.z * w_v3;
